      ...
      <div class="card space-y-4">
        ...
        <div>
          ...
          <select v-model="selectedStudent" class="input">
            <option :value="null" disabled>Seleccionar alumno...</option>
            <option v-for="s in students" :key="s.id" :value="s.id">{{ s.nombre }} ({{ s.email }})</option>
          </select>
        </div>

        <div>
          <label class="label">Rutina</label>
          <select v-model="selectedRoutine" class="input">
            <option :value="null" disabled>Seleccionar rutina...</option>
            <option v-for="r in routines" :key="r.id" :value="r.id">{{ r.nombre }}</option>
          </select>
        </div>

        <AppButton :loading="assigning" @click="handleAssign" :disabled="!selectedStudent || !selectedRoutine">
          Asignar Rutina
        </AppButton>
      </div>
        ...
        <div v-else class="space-y-2">
          <div v-for="a in studentAssignments" :key="a.id"
            ...
            <div>
              ...
              <p class="text-xs text-dark-500">{{ formatDate(a.fechaAsignacion) }}</p>
            </div>
